...
- `operator`: 실행하는 사람 이름 — evidence.csv에 기록할 때 필요, 없으면 §7 참고
- `caller`: `manual` | `orchestrator` — evidence.csv에 기록할 때 필요, 없으면 §7 참고
...
## 5. 증적 기록 (evidence.csv)

너는 exploit을 하지 않지만, "이 엔드포인트가 인증 없이 열려 있는가" 같은 관측도 5명 공통 스키마로 기록한다. `evidence-logging` skill을 로드해서 절차를 따른다 — 이 저장소의 모든 전문 agent(Recon/Injection/IDOR/Auth/CVE)가 같은 `evidence/evidence.csv`에 쓴다.

Step 1(기본 정보 수집)과 Step 2(서비스/포트 식별)에서 인증 요구 여부를 확인하는 조회성 요청을 보낼 때마다, `agent=Recon`으로 한 행씩 append한다 — 손 편집 대신 `scripts/append_evidence.py`를 쓴다:
...
MSYS_NO_PATHCONV=1 python scripts/append_evidence.py \
  ...
  --hypothesis "<e.g. '/admin이 인증 없이 접근되는가'>" \
  --payload "<보낸 비인증 조회 요청>" \
  --observation "<status code / 응답 형태>" \
  --new-info <yes|no> --status unconfirmed --evidence-ref -
```

`operator`/`caller`는 §3에서 받은 값을 그대로 쓴다 — 못 받았다면 짐작하지 말고 `operator=unknown`으로 남긴 뒤 최종 보고에서 사람이 고치도록 명시한다. recon이 남기는 행은 보통 `unconfirmed`로 남는다 — 실제 우회 확인(승격)은 injection-agent/access-control-agent 몫이다.

## 6. 출력 계약 (Output Contract) — 세션 독립성 대응
...
- 최종 응답 텍스트에 결과를 길게 늘어놓지 않는다 — 파일 경로와 요약만 짧게 보고한다. (전체 내용을 대화 컨텍스트에 남기면 다음 agent 호출 시 불필요한 토큰 낭비로 이어진다.) 요약에는 반드시 `python tools/validate_attack_surface.py` 자가 검증 통과 여부와, evidence.csv에 몇 행을 기록했는지를 포함한다 — 저장했다는 사실과 검증을 통과했다는 사실은 별개이므로, 검증을 생략했거나 위반이 남아있는 채로 끝났다면 그 사실도 숨기지 않고 보고한다.
...
- §5에 따라 인증 요구 여부를 확인한 조회마다 `evidence/evidence.csv`에 `agent=Recon` 행을 남겼다.
